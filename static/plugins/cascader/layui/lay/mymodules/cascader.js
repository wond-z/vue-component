...
layui.define(["jquery","laytpl","layer"], function (exports) {
    ...
    Cascader.prototype={
        ...
        initChild: function (triggerData,node) {
            ...
            this.textArr.push(blockData.name);
            this.valueArr.length=this.floor;
            this.valueArr.push(blockData.OID);
            var string =  laytpl(
                '<ul class="urp-cascader-child">'+
                    '{{# for(var i=0;i< d.length;i++){ }}'+
                        '<li>{{ d[i].name }}<i class="layui-icon layui-icon-right"></i></li>'+
        ...
        finishInitData: function (triggerData) {
            ...
            this.valueArr.length=this.floor;
            this.valueArr.push(this.blockData.OID);
            // 文本拼接
            ...
            if(triggerData!=="initValue" && this.option.success) this.option.success(this.valueArr,this.textArr);
            // this.count++;
                // if($.isArray(this.option.OID) && this.option.OID.length>0 && this.count===1 && this.option.success){
            //     return;
            // }
        ...
        initValue: function() {
            var self=this;
            if($.isArray(this.option.OID) && this.option.OID.length>0) {
                if(self.option.lazy) throw "懒加载暂不支持赋初值"
                var value=self.option.OID;
                ...
                //     if(d[arrr[0]].children[i].OID==value[1]){
                //         arrr.push(i)
                ...
                //     if(d[arrr[0]].children[arrr[1]].children[i].OID==value[2]){
