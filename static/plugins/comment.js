...
function comment(commentUrl, attr, name2, src, host) {
    ...
    function reply(box, el) {
        ...
        serverFiles.forEach(function (v) {
            if (v.name) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.code === 200) {
                            ...
                            if (res.code === 200) {
                                res.data.forEach(function (item) {
                                    filesHtml += `<tr>
                                                ...
                                                <td>
                                                <a href=${host}${item.original}>下载</a>
                                                </td>
                                                </tr>
                                                `
                                })
                    ...
                    atrenList = []
                    var commentBox = `
                     <div class="comment-box clearfix" id=${res.data[0].OID} data-ver=${res.data[0].version} data-pid=${res.data[0].OID} data-atid=${res.data[0].addUserOID} data-name= ${name2}>
    ...
    function sc(id, ver) {
        ...
        layer.confirm('您确定要删除吗?', {
            ...
            $.ajax({
                url: "/admis/projectCollaborationNoticeComment/DeleteComment.rest",
                data: {
                    commentOID: id,
                    version: ver,
                    userOID: _user_id
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.code === 200) {
                    ...
                    var fileListHtml = `<table class="comment-files"><tbody>
                                            ${filesHtml}
                                        </tbody> </table>`
                    atrenList = []
                    var commentBox = `
                            <div class="comment-box clearfix" id=${res.data[0].OID} data-ver=${res.data[0].version} data-pid="${res.data[0].parentOID}" data-atid="${res.data[0].addUserOID}" data-name= ${name2}>
                            <img class="myhead" style="width: 30px; height: 30px" src= ${src}>
                            <div class="comment-content">
                            <div class="comment-text">
                                <p class="user">  ${name2}</p>
...
//把事件代理到每条分享div容器
    for (var i = 0; i < boxs.length; i++) {
        ...
        boxs[i].onclick = function (e) {
            ...
            switch (el.className) {
                //评论
                case 'fb-action layui-btn layui-btn-sm layui-btn-normal':
